...
trait Codec[Req, Rep] {
  ...
  def newServerDispatcher(
    ...
    f: (Any) => TraceId = AnnotatedTransport.AnyToTraceId): Closable = {
      val trans = new AnnotatedTransport(Transport.cast[Rep, Req](transport), f)
      new SerialServerDispatcher[Req, Rep](trans, service)
  }
