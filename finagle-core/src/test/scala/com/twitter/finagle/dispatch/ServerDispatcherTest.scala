...
import org.mockito.Mockito.{when, never, verify, times, atLeast}
...
class SerialServerDispatcherTest extends FunSuite with MockitoSugar {
  ...
  test("WireSend and WireRecv Annotations") (new Ctx {
    ...
    Trace.letTracer(tracer) {
      ...
      val config = new ServerDispatcherConfig[String](g, true)
      val disp = new SerialServerDispatcher(trans, service, config, true)
     ...
     verify(tracer, atLeast(2)).record(any[Record])
  })
